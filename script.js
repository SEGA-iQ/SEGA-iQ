...
const currentDataVersion = '4.0'; // قم بتغيير الإصدار عند تحديث البيانات
...
const handleOrderSubmissionDebounced = debounce(handleOrderSubmission, 2000);

document.getElementById('submitOrder').addEventListener('click', handleOrderSubmissionDebounced);
...
async function handleOrderSubmission() {
    ...
    if (submitButton.disabled) return;  // الخروج إذا كان الزر معطلاً بالفعل

    submitButton.disabled = true;  // تعطيل الزر لمنع الضغط المتكرر
    ...
    try {
        ...
        showErrorMessage("Error sending order, please try again.");
    } finally {
        hideLoadingIndicator();  // إخفاء شاشة التحميل
        submitButton.disabled = false;  // إعادة تمكين الزر بعد الإرسال
    }
}
